...
y <- seq(0.05, 3, length = 1000) #theta2
...
x <- seq(0.001, 0.2, length = 1000) #theta1
y <- seq(0.05, 3, length = 1000) #theta2
z <- outer(x, y, loglikelihood.full.data)
...
#Get a plots of contour

```{r}


pdf("myContour-with-MLE.pdf", width = 8, height = 6)
...
text(
  ...
  y     = theta2.hat,
  labels = bquote(
  theta^{"true"} == "(" ~ .(round(theta1.hat, 3)) ~ "," ~ .(round(theta2.hat, 3)) ~ ")"
),
  pos   = 4,
  col   = "blue"
)
...
# add the confident interval with the chi-square approximation.

```{r}

# Example grid
theta1.seq <-seq(0.001, 0.2, length.out = 300)  # theta1
theta2.seq <- seq(0.05, 2,    length.out = 300)  # theta2
...
region_line <- contourLines(theta1.seq, theta2.seq, dev, levels = crit_val)
